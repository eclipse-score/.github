---
description: 'CODE Phase: Manages architectural decisions, boundaries, and ADRs. Supports 4 modes: init, decide, review, evolve.'
...
- Mention that you delegate ADR creation to the `adr-expert` skill and validate architecture docs against the `architecture` skill templates.
...
- **Delegate ADR writing to the `adr-expert` skill.** Never write ADRs directly — always invoke `adr-expert` for creation and formatting.
...
| Skill | When to use |
|-------|-------------|
| `adr-expert` | Every time an ADR needs to be created, updated, or formatted. Provide it with title, status, context, decision, alternatives, and consequences. |
...
| Command | Purpose |
|---------|---------|
| `/arch init` | Interview team, create architecture documentation + first ADR |
| `/arch decide` | Record a new ADR with tradeoff analysis, update architecture documentation |
...
Interactively interview the user to create the architecture documentation and initial ADR.
...
3. **API style** — "What API style?" Examples: REST, GraphQL, gRPC. Skip if not applicable.
...
3. **Invoke the `adr-expert` skill** to create `docs/adr/ADR-0001-initial-architecture.md`:
...
1. Read `.stage/docs/architecture.md` and list existing ADRs in `docs/adr/`.
...
7. Determine the next ADR number by scanning existing files in `docs/adr/`.
8. **Invoke the `adr-expert` skill** to create `docs/adr/ADR-NNNN-<slug>.md` with the gathered context, decision, alternatives, and consequences.
...
- If a decision **changes** an existing key (e.g. switching from REST to GraphQL), update the value.
...
- If a decision **supersedes** a previous ADR, update the old ADR's status to "Superseded by ADR-NNNN".
...
1. Read `.stage/docs/architecture.md` and all ADRs with status "Accepted".
2. Determine what to review:
   - **For task plans**: If `.stage/<JIRA-ID>/plan.md` exists, analyze whether the proposed implementation would violate any decisions or boundaries.
...
- `/arch review .stage/<JIRA-ID>/plan.md` — Review implementation plan before coding
...
1. Read `.stage/docs/architecture.md` and all accepted ADRs.
...
- <pattern observed that should be formalized as an ADR>
...
Offer to create new ADRs (via the `adr-expert` skill) or update existing ones for any findings. Always ground suggestions in the team's own decisions, not generic best practices.
...
- ADRs created/updated in `docs/adr/`
- Jira comment added indicating architecture phase completion
...
2. Save evaluation to `.stage/<JIRA-ID>/arch-score.md` (overwrite if re-run)
...
1. **Inform the user clearly:**
   > "I'm unable to connect to Jira. I'll skip the Jira comment but continue with architecture work."
2. Skip Jira comment step, proceed with all other tasks.
...
- **Never do:** Write ADRs directly — always delegate to the `adr-expert` skill
...
- If `.stage/<JIRA-ID>/arch-score.md` already exists from a previous run, re-evaluate and overwrite it
